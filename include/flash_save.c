...
//////////////////////////////////////////////////
		while(1)
		{
			...
			{
				Address = FLASH_START_ADDR + (page * FLASH_PAGE_SIZE) + (index*4); // вычисл€ем адрес начала страницы
				return 	(*(__IO uint32_t*)Address); // „итаем данные из флеша
				//return 	(index*4); // „итаем данные из флеша
				//return 	Address; // „итаем данные из флеша
			}
...
//////////////////////////////////////////////////
		while(1)
		{
			if(index<doze_length) // Ќашли нужную страничку
			{
				Address = FLASH_START_ADDR + (FLASH_PAGE_SIZE/2) + (page * FLASH_PAGE_SIZE) + (index*4); // вычисл€ем адрес начала страницы
				return 	(*(__IO uint32_t*)Address); // „итаем данные из флеша
				//return 	(index*4); // „итаем данные из флеша
				//return 	Address; // „итаем данные из флеша
			}
