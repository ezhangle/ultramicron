...
#else // версия платы без индуктивности
TIM_PrescalerConfig(TIM10,(uint32_t) (SystemCoreClock / 32000) - 1,TIM_PSCReloadMode_Immediate); // частота таймера 32 кГц
#endif
...
#ifdef version_330 // версия платы с индуктивностью
  TIM_BaseConfig.TIM_Prescaler = (uint32_t) (SystemCoreClock / 524250) - 1; // частота таймера ~524.2 кГц
  TIM_BaseConfig.TIM_Period = 65;  // ~8 кГц
  TIM_OCConfig.TIM_Pulse = 2; // Скваженность ~3% (ток около 16.8мА)
#else // версия платы без индуктивности
  TIM_BaseConfig.TIM_Prescaler = (uint32_t) (SystemCoreClock / 32000) - 1; // частота таймера 32 кГц
  TIM_BaseConfig.TIM_Period = 4;  // ~8 кГц
  TIM_OCConfig.TIM_Pulse = 2; // Скваженность ~50% 
#endif
  // Как я понял - автоматическая перезарядка таймера, если неправ - поправте.
