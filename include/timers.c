...
TIM_OCInitTypeDef TIM_OCConfig;
  ...
  TIM_BaseConfig.TIM_Prescaler = (uint32_t) (SystemCoreClock / 500000) - 1; // частота таймера ~524.2 кГц
  ...
  TIM_BaseConfig.TIM_Prescaler = (uint32_t) (SystemCoreClock / 16000) - 1; // частота таймера 16 кГц
  TIM_BaseConfig.TIM_Period = 2;  // ~8 кГц
  TIM_OCConfig.TIM_Pulse = 1; // Скваженность ~50% 
#endif
  // Как я понял - автоматическая перезарядка таймера, если неправ - поправте.
