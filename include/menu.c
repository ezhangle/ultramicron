...
MenuItem Menu_list[max_struct_index] = {
  ...
  {  0x00, "Звук",	        "откл",							"вкл",	        	"",	           &Settings.Sound,                                        0,       1,        0,       &plus_on,                 &minus_off},
	{  0x00, "Режим USB",     "*",    						"*",	          	"*",           0x00,                                                   0x00,    0x00,     0x00,    &usb_activate,            &usb_deactivate},
...
//  {  0x00, "Выключение",    "*",		  					"*",		        	"*",	         0x00,                                                   0x00,    0x00,     0x00,    &plus_poweroff,           0x00},
  ...
  {  0x01, "Реверс",	      "откл",							"",			        	"%u",	         &Settings.Display_reverse,                              0,       3,        0,       &plus_one,                &minus_one},
	...
	{  0x01, "Индукция",	    "",		  						"",			        	"%uмТл",	     &Settings.Pump_Energy,                                  350,     450,      300,     &plus_50,                 &minus_50},
  {  0x01, "Счет",	        "",		  						"",			        	"%uсек",	     &Settings.Second_count,                                 200,     450,      200,     &plus_ten,                &minus_ten},
	{  0x01, "LSI",		        "",		  						"",			        	"%u Гц",	     &Settings.LSI_freq,                            	       26000,   56000,    38000,   &plus_500,                &minus_500},
  {  0x01, "Звук",	        "",		  						"",			        	"%uкГц",	     &Settings.Sound_freq,                                   1,       10,       8,       &plus_one,                &minus_one}
//  {  0x01, "Накачка",	      "",		  						"",			        	"%uимп",	     &Settings.second_pump,                                  1,       60,       10,      &plus_one,                &minus_one},
//  {  0x01, "Нач. плато",	  "",									"",			        	"%uВ",		     &Settings.Geiger_plato_begin,                           250,     450,      320,     &plus_one,                &minus_one},
//  {  0x01, "Плато",	        "",									"",			        	"%uВ",		     &Settings.Geiger_plato,                                 20,      200,      80,      &plus_one,                &minus_one},
...
//  LcdString(1,5); // // Выводим обычным текстом содержание буфера на строку 8
  ...
  else LcdBatt(82, 19, 82+10, 19+19, battery_procent);//рисуем батарейкуADCData.Batt_voltage

	if (main_menu_stat>4)main_menu_stat=1;
	if (main_menu_stat<1)main_menu_stat=4;
  ...
  // -----------------------------------------
		case 0x04:
			...
			if(Doze_massive[doze_length]>0)
			{
				sprintf (lcd_buf, "%9u мкР", (Doze_count*(Settings.Second_count>>2))/900); // Пишем в буфер значение счетчика
			} else {
				sprintf (lcd_buf, " *расчет* мкР"); // Пишем в буфер значение счетчика
			}
