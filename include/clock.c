...
void set_msi(FunctionalState sleep)
{
   ...
   RCC_MSICmd(ENABLE); // ¬ключить MSI
   while(RCC_GetFlagStatus(RCC_FLAG_MSIRDY)==RESET); // ∆дем включени€ MSI
...
switch (RCC_GetSYSCLKSource())
{
  ...
  case 0x04: // HSI, переконфигурировать
   RCC_SYSCLKConfig(RCC_SYSCLKSource_MSI); // ”становить источником тактировани€ MSI
   while(RCC_GetSYSCLKSource() != 0x00); // ѕодождать пока произойдет смена
   RCC_HSICmd(DISABLE);  // ќтключить HSI
  ...
  case 0x08: // HSE, переконфигурировать
   RCC_SYSCLKConfig(RCC_SYSCLKSource_MSI); // ”становить источником тактировани€ MSI
   while(RCC_GetSYSCLKSource() != 0x00); // ѕодождать пока произойдет смена
   RCC_HSEConfig(RCC_HSE_OFF);  // ќтключить HSE
  ...
  case 0x0C: // PLL, переконфигурировать
   RCC_SYSCLKConfig(RCC_SYSCLKSource_MSI); // ”становить источником тактировани€ MSI
   while(RCC_GetSYSCLKSource() != 0x00); // ѕодождать пока произойдет смена
   RCC_PLLCmd(DISABLE);  // ќтключить PLL
